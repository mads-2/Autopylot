...
class Grader:
    ...
    def append_score_columns_to_df(self):
        self.data['Spectra AUC score'] = self.interval_auc_overlap()
        self.data['Total score'] = self.data['Spectra AUC score'] + self.data['Time score']
        self.data.sort_values(by=['Total score'], ascending=False, inplace=True, ignore_index=True)

        print("Method Scores:")
        print(self.data[['Method', 'Spectra AUC score', 'Time score', 'Total score']])
    ...
    def interval_auc_overlap(self):
        ...
        for idx, row in self.data.iterrows():
            ...
            alphas_for_candidate = suggested_alphas[idx]
            overlap_per_candidate = []

            # Apply each alpha, and shift all states
            for i, alpha in enumerate(alphas_for_candidate):
    ...
    def make_spectra(self):
        ...
        E = np.linspace(energy_min, energy_max, 1000)  # Raw energy range
        sigma = 0.15  # HWHM
        ...
        # Extract and build the reference spectrum (raw energy)
        ...
        for state in range(1, self.n_singlets):
            ...
            osc_value = ref_row.get(osc_col)
            if pd.notna(ref_row[energy_col]) and pd.notna(osc_value):
                x0 = ref_row[energy_col]  # Raw energy
                ref_spectrum += gauss(E, osc_value, x0, sigma)  # Keep y-axis values untouched
        ...
        for idx, row in self.data.iterrows():
            ...
            # Set up the plot
            plt.figure(dpi=300)
            plt.plot(E, ref_spectrum, label="EOM-CC2", linestyle='--', color='black')

            # Define a color cycle for the different alphas
            color_cycle = plt.cm.viridis(np.linspace(0, 1, len(alphas_for_candidate)))
            ...
            for i, alpha in enumerate(alphas_for_candidate):
                ...
                # Plot the spectrum for the current alpha
                plt.plot(E, spectrum, label=f"{formatted_method_name} ($S_{i}$) (alpha={alpha:.2f})", color=color_cycle[i])

            # Add labels, legend, and title
            plt.xlabel("Energy (eV)")
            plt.ylabel("Absorbance")
            plt.legend(loc='upper right', fontsize=4)
            plt.title(f"UV-Vis Spectra: EOM-CC2 vs {formatted_method_name}")
            plt.grid(False)
    ...
    def format_method_name(self, method_name):
        ...
        if fomo_match:
            ...
            formatted_name = re.sub(r'fomo_T\d+\.\d+', f'FOMO(t0={t0_value})', formatted_name)
        ...
        rc_w_match = re.search(r'rc_w(\d+.\d+)', method_name)
        if rc_w_match:
    ...
    def plot_results(self):
        ...
        if not filtered_data.empty and 'Total score' in filtered_data.columns and not filtered_data['Total score'].isna().all():
            ...
            ax2.plot(xs, filtered_data['Spectra AUC score'], color='r', label='Spectra AUC Score', linewidth=5, marker='8', markersize=20)
    ...
    def AUC_histogram(self):
        ...
        auc_scores = self.data['Spectra AUC score'].dropna()
    ...
    def export_scores_to_txt(self, filename='Final_Scores.txt'):
        ...
        if not self.data.empty:
            ...
            # Columns to export
            columns_to_export = ['Method', 'Spectra AUC score', 'Time score', 'Total score']
...
def main():
    ...
    if 'candidates' in settings:
        for calc_type in settings['candidates']:
            ...
            generator_settings = settings['candidates'][calc_type]
            generator_settings.update({
                'calc_type': calc_type,
                'nelec': nelec,
                'charge': charge,
                'n_singlets': n_singlets,
            ...
            candidate_generator = CandidateListGenerator(**generator_settings)

            # Generate candidates for singlets and triplets
            singlet_candidates = candidate_generator.create_candidate_list()
            triplet_candidates = candidate_generator.create_candidate_list()

            # Combine singlet and triplet candidates
            all_candidates = singlet_candidates + triplet_candidates

            print(f"Generated {len(all_candidates)} candidates for calc_type: {calc_type}")

            for candidate in all_candidates:
                candidate.validate_as()
                print(f"Launching calculations for: {candidate.folder_name}")

                # Build the folder path and check if it exists
                folder_path = fol_name / f'gradient_{candidate.full_method}'
                print(f"Expected folder path: {folder_path}")
    ...
    grader.append_score_columns_to_df()  # Compute Spectra AUC score, Total score, etc.
    grader.make_spectra()
